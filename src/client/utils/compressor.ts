// TODO: Optimize performance - currently slow and memory-intensive for large files.
// Works well for .jar natives but struggles with larger files like Java 8 zip.
// Using adm-zip as a temporary solution.
...
export async function unzipFile(
	...
	if (!existsSync(inputPath)) {
		throw new Error(ERRORS.FILE_NOT_FOUND(inputPath));
	}
	...
	try {
		...
		throw new Error(ERRORS.UNZIP_FAILED(error));
	}
}
